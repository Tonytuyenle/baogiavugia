...
6	LK-2633		Chảo Belly Lock&King Titanium 26cm	450,000 đ	850,000 đ	961,200 đ	1,200,000 đ	400,000 đ	47.1%
...
lines.forEach(line => {
  ...
  if (line.startsWith('STT') || line.includes('BẢNG PHÂN LOẠI') || line.includes('phía trên') || line.includes('65 mã') || line.includes('PHẦN I') || line.includes('PHẦN II')) {
    return;
  }
  ...
  if (parts.length >= 7) {
    ...
    const model = parts[1];
    const name = parts[3];
    const brand = model.startsWith('TK') ? 'Takin' : 'Lock&King';
    ...
    const imgFileName = `${model}.png`;
    const hasImg = fs.existsSync(path.join(__dirname, 'images', imgFileName));
    const image = hasImg ? `images/${imgFileName}` : '';
...
fs.writeFileSync(path.join(__dirname, 'real_catalog_v4.json'), JSON.stringify(newCatalog, null, 2), 'utf-8');

// Also create the formatted Excel sheet with section headers
const excelRows = [
  ['BẢNG PHÂN LOẠI GIÁ – KÊNH NPP ONLINE LOCK&KING & TAKIN'],
  ['65 Mã Sản Phẩm Chuẩn | Giá Và Lợi Nhuận Tự Động Tham Chiếu Từ Danh Mục Gốc'],
  []
];
...
newCatalog.forEach((p, idx) => {
  if (p.brand !== lastBrand) {
    excelRows.push([]);
    excelRows.push([p.brand === 'Lock&King' ? 'PHẦN I – THƯƠNG HIỆU LOCK&KING' : 'PHẦN II – THƯƠNG HIỆU TAKIN']);
    lastBrand = p.brand;
    lastCategory = '';
  }
  ...
  if (p.categoryName !== lastCategory) {
    excelRows.push([]);
    excelRows.push([`NHÓM: ${p.categoryName}`]);
    excelRows.push([
      'STT', 'Model', 'Hình Ảnh', 'Tên Sản Phẩm', 'Hãng', 
      'Giá Nhập Cũ (VNĐ)', 'Giá Nhập Mới / Áp Dụng (VNĐ)', 'Chênh Lệch Nhập (VNĐ)', '% Biến Động Vốn',
      'Giá NPP Online (VNĐ)', 'Giá Facebook (VNĐ)', 
      'Giá Sàn TMĐT (VNĐ)', 'Giá Niêm Yết (VNĐ)', 'Lợi Nhuận NPP (VNĐ)', '% LN NPP / Facebook',
      'Giá NPP Offline (VNĐ)', 'Chênh Lệch Off - On (VNĐ)', '% Lệch Off/On', 'Giá Đại Lý Offline (x120%)',
      '% LN Đại Lý / Facebook'
    ]);
  ...
  excelRows.push([
    ...
    p.nppOfflinePrice,
    p.diffOnlineOffline,
    (p.diffOnlineOffline > 0 ? '+' : '') + p.diffOnlineOfflinePct,
    p.dealerOfflinePrice,
    p.dealerProfitRatio
  ]);
});
...
ws['!cols'] = [
  ...
  { wch: 42 }, // Tên SP
  { wch: 12 }, // Hãng
  { wch: 18 }, // Giá NPP Online
  { wch: 18 }, // Giá Facebook
  { wch: 18 }, // Giá TMĐT
  { wch: 18 }, // Giá Niêm Yết
  { wch: 18 }, // Lợi Nhuận NPP
  { wch: 18 }, // % LN NPP / FB
  { wch: 18 }, // Giá NPP Offline
  { wch: 20 }, // Chênh lệch Off - On
  { wch: 16 }, // % Lệch Off/On
  { wch: 22 }, // Giá Đại Lý Offline
  { wch: 20 }, // % LN Đại Lý
  { wch: 18 }  // Vốn
];

XLSX.utils.book_append_sheet(wb, ws, 'BangGiaNPPOnline');
...
console.log('Exported clean Excel with section dividers!');
