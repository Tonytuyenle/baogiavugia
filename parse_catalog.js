...
const rawData = `STT	Model	Hình ảnh	Tên sản phẩm	Giá NPP Online	Giá Facebook	Giá sàn TMĐT	Giá niêm yết	Lợi nhuận NPP	% LN NPP / Facebook
...
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];
  if (line.includes('PHẦN II – TAKIN')) {
    currentBrand = 'Takin';
    continue;
  }
  if (['CHẢO', 'NỒI & BỘ NỒI', 'NỒI TĂNG ÁP', 'ẤM & BÌNH', 'ĐỒ ĐIỆN'].includes(line)) {
  ...
  if (parts.length >= 8 && !isNaN(parseInt(parts[0]))) {
    ...
    const fbPrice = cleanPrice(parts[5]);
    const tmdtPrice = cleanPrice(parts[6]);
    const retailPrice = cleanPrice(parts[7]);
    const profit = cleanPrice(parts[8]) || (fbPrice - nppOnline);
    const profitPct = parts[9] || ((profit / fbPrice) * 100).toFixed(1) + '%';
    ...
    products.push({
      ...
      nppOnlinePrice: nppOnline,
      facebookPrice: fbPrice,
      tmdtPrice: tmdtPrice,
      retailPrice: retailPrice,
      profitNPP: profit,
      profitPct: profitPct,
      costPrice: nppOnline, // Giá vốn của NPP bán online
      dealerPrice: fbPrice,
      distributorPrice: nppOnline,
      unit: currentCategory.includes('BỘ') ? 'Bộ' : (currentCategory.includes('NỒI') || currentCategory.includes('CHẢO') ? 'Cái' : 'Chiếc'),
      status: 'ACTIVE'
    });
  }
}

console.log(`Parsed successfully ${products.length} products!`);
fs.writeFileSync(path.join(__dirname, 'real_catalog.json'), JSON.stringify(products, null, 2), 'utf-8');

// Also create CSV file
let csvContent = '\uFEFFSTT,Thương Hiệu,Danh Mục,Mã Sản Phẩm (Model),Tên Sản Phẩm,Giá NPP Online (VNĐ),Giá Facebook (VNĐ),Giá Sàn TMĐT (VNĐ),Giá Niêm Yết (VNĐ),Lợi Nhuận NPP (VNĐ),% LN NPP / FB\n';
products.forEach(p => {
  csvContent += `"${p.stt}","${p.brand}","${p.categoryName}","${p.canonicalCode}","${p.name.replace(/"/g, '""')}","${p.nppOnlinePrice}","${p.facebookPrice}","${p.tmdtPrice}","${p.retailPrice}","${p.profitNPP}","${p.profitPct}"\n`;
});
fs.writeFileSync(path.join(__dirname, 'bang_gia_chuan_lock_and_king_takin.csv'), csvContent, 'utf-8');
console.log('Created bang_gia_chuan_lock_and_king_takin.csv');
